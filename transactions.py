...
def process_transactions_dataframe(df):
    ...
    df.columns = ['Date', 'Product', 'ISIN', 'Exchange', 'Number',
            'Price' , 'Currency', 'Cost (Exchange)', 'Currency (Exchange)',
            'Cost (Local)', 'Currency (Local)', 'Exchange Rate',
            'Commission', 'Currency (Comission)', 'Total Cost',
            'Currency (Total)']
...
def check_valid_ticker_data(tickers, ticker):
    ...
    if not hist.empty and hist.Close.value_counts().max() < 30 and len(hist) > 200 and len(tickers.tickers[ticker].info) > 10:
        return 1
    else:
        return 0
...
def get_ticker_list_data(ticker_list):
    ...
    for ticker in tickers.tickers:
        if check_valid_ticker_data(tickers, ticker):
            valid_entry = {
                    'Data' : tickers.tickers[ticker].history(period='max').reset_index(),
...
def plot_selected_stock_and_start_date(df, start_date, end_date):
    df = df.loc[(df.Date.dt.year >= start_date) & (df.Date.dt.year <= end_date)]
